...
import 'package:date_format/date_format.dart'as matFor;
class DateFormats {
...
class DateUtil {
  ...
  /// 返回两个日期
  static String formatTowDate(DateTime startDate,DateTime endDate){
    String start = matFor.formatDate(startDate,[matFor.yyyy,'-',matFor.mm,'-',matFor.dd]);
    String end = matFor.formatDate(endDate,[matFor.mm,'-',matFor.dd]);
    return "$start - $end";
  }
  /// com format.
  ...
  /// 返回年月日的String
  static String yearMonthDay(DateTime startDate, [DateTime endTime, bool year]) {
    if (endTime == null) {
      return matFor.formatDate(startDate,[matFor.yyyy,'-', matFor.mm, '-', matFor.dd]);
    }
  }
}
